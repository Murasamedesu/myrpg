    ...
    class QuestManager
    {
        ...
        public Result AcceptQuest(NetConnection<NetSession> sender, int questId)
        {
            ...
            if(DataManager.Instance.Quests.TryGetValue(questId, out quest))
            {
                ...
                {
                    dbquest.Status = (int)QuestStatus.InProgress; // 有目标的进入进行状态
                }
        ...
        public Result SubmitQuest(NetConnection<NetSession> sender, int questId)
        {
            ...
            if(DataManager.Instance.Quests.TryGetValue(questId, out quest))
            {
                ...
                if(dbquest != null)
                {
                    if(dbquest.Status != (int)QuestStatus.Complated)  // 任务还没完成
                    {
                ...
                sender.Session.Response.questSubmit.Errormsg = "[10002]任务不存在"; //数据库中不存在
                return Result.Failed;
            }
            else
            {
                sender.Session.Response.questSubmit.Errormsg = "[10001]任务不存在"; //表中不存在
                return Result.Failed;
            }
        }
