...
namespace GameServer.Services
{
    class ChatService : Singleton<ChatService>
    {
        ...
        void OnChat(NetConnection<NetSession> sender, ChatRequest request)
        {
            ...
            Log.InfoFormat("OnChat:: character: {0} Channel: {1} Message: {3}", character.Id, request.Message.Channel, request.Message.Message);
            if(request.Message.Channel == ChatChannel.Private)    // 是否是私聊
            {
